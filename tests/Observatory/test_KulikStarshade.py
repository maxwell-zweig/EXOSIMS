...
class TestKulikStarshade(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        with self.assertRaises(Exception) as context:
            KS(mode="energyOptimals", dynamics=0, exponent=10, precompfname="haloImpulsive", starShadeRadius=10)
        self.assertEqual(str(context.exception), 'Mode must be one of "energyOptimal" or "impuslive"')

        try: 
            KS(mode="energyOptimal", dynamics=0, exponent=10, precompfname="haloImpulsive", starShadeRadius=10)
        except Exception as e:
            self.fail(f"Instantiation raised an exception: {e}")

        try: 
            KS(mode="haloImpulsive", dynamics=0, exponent=10, precompfname="haloImpulsive", starShadeRadius=10)
        except Exception as e:
            self.fail(f"Instantiation raised an exception: {e}")

        
    def test_inert_to_rel(self, t_can, inert_pos):
        pass


    def test_time_to_can(self, t_abs):
        pass 
    ...
    def test_dV(self):
        ...
        starshade = KS(mode="energyOptimal", dynamics=0, exponent=8, precompfname="haloEnergy", starShadeRadius = 10)
        ...
        starshade = KS(mode="impulsive", dynamics=0, exponent=8, precompfname="haloEnergy", starShadeRadius = 10)
        targets = KnownRVPlanetsTargetList.KnownRVPlanetsTargetList(modules= {"StarCatalog": "EXOCAT1", "OpticalSystem": " ", "ZodiacalLight": "  ", "PostProcessing": " ", "Completeness": " ", "PlanetPopulation" : "KnownRVPlanets", "BackgroundSources" : " ", "PlanetPhysicalModel" : " "})
        dV = starshade.calculate_dV(targets, 5, np.array([13, 14, 17, 20]), np.ones((4, 50)) + np.random.rand(4,50), starshade.equinox)
        self.assertTrue(np.all(dV >= 0))
        self.assertTrue(dV.shape == np.ones((4, 50)).shape)
